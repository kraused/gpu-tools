...
def functionPrototype(func, args):
	return "void %s(%s);" % (re.sub(r'^nvml(.*)', r'Nvml_\1', func), ", ".join(args))

def functionDefinition(func, args):
	function = []

	function += ["void %s(%s)" % (re.sub(r'^nvml(.*)', r'Nvml_\1', func), ", ".join(["%s a%d" % (x, i) for i, x in enumerate(args)]))]
	function += ["{"]
	function += ["\tnvmlReturn_t err;"]
	function += ["\terr = %s(%s);" % (func, ", ".join(["a%d" % i for i, _ in enumerate(args)]))]
	function += ["\tnvml_return_handler(\"%s\", \"%s\", err, nvmlErrorString(err));" % (re.sub(r'^nvml(.*)', r'Nvml_\1', func), func)]
	function += ["}"]
