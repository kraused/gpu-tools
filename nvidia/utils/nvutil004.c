...
/* nvutil004: Query API Restrictions for devices.
 */
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < count; ++i) {
		...
		Nvml_DeviceGetAPIRestriction(device, NVML_RESTRICTED_API_SET_AUTO_BOOSTED_CLOCKS, &enabled);
		fprintf(stdout, "device %d auto boosted clocks restrictions: %s\n", i, Nvml_Convert_EnableState(enabled));
	}
