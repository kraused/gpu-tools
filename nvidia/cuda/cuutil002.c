...
{
	Nvcu_DeviceGet(&Ctx.gpuDev, GPU_DEVICE);
	Nvcu_CtxCreate(&Ctx.gpuCtx, 0, Ctx.gpuDev);
}
...
void alloc()
{
	...
	Nvcu_MemAlloc(&p, BUF_NUM_PAGES*Ctx.pgsize);
	Ctx.gpuBuf = (void *)p;

	flag = 1;
	Nvcu_PointerSetAttribute(&flag, CU_POINTER_ATTRIBUTE_SYNC_MEMOPS, (CUdeviceptr )Ctx.gpuBuf);

	Nvcu_Memcpy((CUdeviceptr )Ctx.gpuBuf, (CUdeviceptr )Ctx.hostBuf, BUF_NUM_PAGES*Ctx.pgsize);
...
void sync()
{
	Nvcu_Memcpy((CUdeviceptr )Ctx.hostBuf, (CUdeviceptr )Ctx.gpuBuf, BUF_NUM_PAGES*Ctx.pgsize);
}
